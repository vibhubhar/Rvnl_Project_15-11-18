...
function mywinClose() {
	...
	$(document).ready(function() {
		...
		var docType1=document.getElementById("1");
		if(docType1==null){
			docType1="";
		}
		else{
		...
		if(tc1==null){
			...
			tc1="Pending with Current User for last :"+tc1+" days";
		}
		
		var tc21=document.getElementById("tc21");
		var tc22=document.getElementById("tc22");
		
		if(tc21==null && tc22==null){
		var	tc2="";
		}
		else{
			var tc21=document.getElementById("tc21").value;
			var tc22=document.getElementById("tc22").value;
			var	tc2="Pending with Current User for  > :"+tc21+ " days & <= :"+tc22+" days";
		}
		...
		if(tc3==null){
			...
			tc3="Pending with Current User for  more than :"+tc3+" days";
		}
	    ...
	    var pkl3=dept1+"      "+suser1+"      "+t3_1+"      "+tc1+"     "+tc2+"      "+tc3+"               "+majsec1+"               "+smajsec1;
...
{exportOptions: {
    columns: [':visible :not(:last-child)']
		                 ...
		                 title: 'RVNL Document Detailed Report for a Department',
		                  extend: 'excel',
		                  ...
		                  customize: function (xlsx) {
		                      ...
		                      //insert
		                      var r1 = Addrow(1, [{ key: 'F', value:  'RVNL Document Detailed Report for a Department' }]);
...
<body onload="mywinClose();">
	<div style="text-align: center;font-size: 18px;"><b>RVNL Document Detailed Report for a Department</b></div>
	<%
	...
	String docType=null;
		...
		if (tcode.equalsIgnoreCase("5")) {
			
			%>
			<div style="text-align: center;font-size: 14px;"><b>(for documents cleared  by the department)</b></div><br><br>
			<%} else { %>
			<div style="text-align: center;font-size: 14px;"><b>(for documents pending with users of the department)</b></div><br><br>
			<%} 	
	...
	<%
				...
				if(sDate!=""){
					...
					<%
						...
						if(dept!=""){
							header=header+"  "+"Department :  "+dept+" ";
					%>
					<th width="250px" align="left" style="background-color: #ffff;"><font size=3px>
							Department :  <b> <%=dept%></b>
							<INPUT Type="hidden" name="dept1" id="dept1" value="<%=dept %>" />
					</font></th>
					<%
					}
			...
			<%
				...
				if(tcode.equalsIgnoreCase("1")){
					header=header+"  "+"Under Approval for last :"+t1+" days";
			%>
			<th width="250px" align="left" style="background-color: #ffff;"><font size=3px>
					Under Approval for last:  <b> <%=t1%></b>&nbsp; days
					<INPUT Type="hidden" name="tc1" id="tc1" value="<%=t1 %>" />
				</font></th>
			<%
			}
				if(tcode.equalsIgnoreCase("2")){
					header=header+"  "+"Under Approval for > :"+t1+" days & <= :" +t2+" days";
			%>
			<th width="250px" align="left" style="background-color: #ffff;"><font size=3px>
					Under Approval for > :  <b> <%=t1%></b> &nbsp; days  & <= :  <b> <%=t2%>&nbsp;days</b>
			<INPUT Type="hidden" name="tc21" id="tc21" value="<%=t1 %>" />	
			<INPUT Type="hidden" name="tc22" id="tc22" value="<%=t2 %>" />	
			</font></th>
			<%
			}
				if(tcode.equalsIgnoreCase("3")){
					header=header+"  "+"Under Approval for more than :  "+t2+" days";
			%>
			<th width="250px" align="left" style="background-color: #ffff;"><font size=3px>
					Under Approval for more than:  <b> <%=t2%></b>&nbsp; days
	...
	<%
		...
		String query = "select DISTINCT docm.Document_Map_ID,projm.PIU_of_RVNL,projm.Project,projm.Project_Coordinator,pkgm.package_name,projm.CoordinatingDepartment,projm.Designation,docm.Document_Type,docm.Document_Number,docm.Document_Name,docm.Date_of_Submission,docm.Document_status,ram.CurrentDate,ram.ActionPerformedBy,DATEDIFF(CURDATE(),docm.Date_of_Submission) as Aging,bmax.date1,BMR.Department, DATEDIFF(ifnull(ram.ActionedOn,now()),ifnull(ram.CurrentDate,docm.date_of_submission)) as pendingsince, BMR.ActionPerformed, BMR.tot_aging, docm.Zone,BMR.division, loc1.Major_Section, docm.From_Km, docm.To_Km, loc1.Station_Name, loc1.Station_Km from xx_projectattributes_master projm,xx_packages_project pkgm,xx_document_mapping docm,xx_approval_workflow wfm,xx_record_actions ram,(select division,  department, ActionPerformed, document_id, sum(BMR_aging) as tot_aging  from (select if(b.type<>'MOR', if(c.division_hierarchyId <>null,c.division, 'Headquarters'), e.division_name) as division, if(b.type='RVNL','RVNL',if(b.Type='Others', om.Org_Name,d.Department_Name)) as Department, a.ActionPerformed, a.Document_id, DATEDIFF(ifnull(a.ActionedOn,now()),ifnull(a.CurrentDate,c.date_of_submission)) as BMR_Aging from xx_record_actions a left join xx_user_master b on a.ActionPerformedBy=b.User_name left join xx_document_mapping c on a.Document_id=c.Document_Map_ID left join xx_departments_master d on b.Department_id=d.Department_id left join xx_divisions_map e on b.Division_ID=e.division_id left join xx_org_master om on b.Org_ID = om.Org_Id ) as BMR1  group by division,  Department, ActionPerformed, document_id) BMR, (select distinct((select max(ram1.Rownum) date1 from xx_record_actions ram1 where ram1.document_id = ram.document_id)) as date1,ram.Document_id from xx_record_actions ram) bmax, (select a.Document_Map_ID, b.Major_Section, c.Station_Km, c.Station_Name from xx_document_mapping a left join xx_project_km b on a.Majsec_Id = b.Majsec_Id left join xx_project_station c on a.Stn_Id = c.Stn_Id ) loc1 where projm.project_id = docm.project_id and docm.Document_Map_ID = loc1.Document_Map_ID and ram.Document_id = wfm.Document_id and docm.document_Map_id=BMR.document_Id and ram.Rownum = bmax.date1 and projm.Project_ID = pkgm.Project_ID and pkgm.package_id = docm.Package_ID and docm.Document_Map_ID = ram.Document_id and docm.Document_Map_ID=wfm.Document_id and bmax.Document_id= ram.Document_id and docm.ShowInReport='y' ";
				//"select DISTINCT docm.Document_Map_ID,projm.PIU_of_RVNL,projm.Project,projm.Project_Coordinator,pkgm.package_name,projm.CoordinatingDepartment,projm.Designation,docm.Document_Type,docm.Document_Number,docm.Document_Name,docm.Date_of_Submission,docm.Document_status,ram.CurrentDate,ram.ActionPerformedBy,DATEDIFF(CURDATE(),docm.Date_of_Submission) as Aging,bmax.date1,BMR.Department, DATEDIFF(ifnull(ram.ActionedOn,now()),ifnull(ram.CurrentDate,docm.date_of_submission)) as pendingsince, BMR.ActionPerformed, BMR.tot_aging, docm.Zone,BMR.division, loc1.Major_Section, docm.From_Km, docm.To_Km, loc1.Station_Name, loc1.Station_Km from xx_projectattributes_master projm,xx_packages_project pkgm,xx_document_mapping docm,xx_approval_workflow wfm,xx_record_actions ram,(select division,  department, ActionPerformed, document_id, sum(BMR_aging) as tot_aging  from (select if(b.type='RVNL', c.division, e.division_name) as division, if(b.type='RVNL','RVNL',d.Department_Name) as Department, a.ActionPerformed, a.Document_id, DATEDIFF(ifnull(a.ActionedOn,now()),ifnull(a.CurrentDate,c.date_of_submission)) as BMR_Aging from xx_record_actions a left join xx_user_master b on a.ActionPerformedBy=b.User_name left join xx_document_mapping c on a.Document_id=c.Document_Map_ID left join xx_departments_master d on b.Department_id=d.Department_id left join xx_divisions_map e on b.Division_ID=e.division_id) as BMR1 group by division,  Department, ActionPerformed, document_id) BMR, (select distinct((select max(ram1.Rownum) date1 from xx_record_actions ram1 where ram1.document_id = ram.document_id)) as date1,ram.Document_id from xx_record_actions ram) bmax, (select a.Document_Map_ID, b.Major_Section, c.Station_Km, c.Station_Name from xx_document_mapping a left join xx_project_km b on a.Majsec_Id = b.Majsec_Id left join xx_project_station c on a.Stn_Id = c.Stn_Id ) loc1 where projm.project_id = docm.project_id and docm.Document_Map_ID = loc1.Document_Map_ID and ram.Document_id = wfm.Document_id and docm.document_Map_id=BMR.document_Id and ram.Rownum = bmax.date1 and projm.Project_ID = pkgm.Project_ID and pkgm.package_id = docm.Package_ID and docm.Document_Map_ID = ram.Document_id and docm.Document_Map_ID=wfm.Document_id and bmax.Document_id= ram.Document_id and docm.ShowInReport='y' ";
		...
		if (dept != "") {
			query=query+" and BMR.Department='"+dept+"'";
		}
		if(sDate!="" && eDate!=""){
...
System.out.println("from stn km"+frmstnkm+"to km"+tostnkm);
	...
	if (t3 == "" || t3 =="0" || t3==null) { } else {
		query=query+" and BMR.ActionPerformed='pending' and DATEDIFF(CURDATE(),docm.Date_of_Submission)>="+ Integer.parseInt(t3)+" ";
	}
	if (tcode.equalsIgnoreCase("1")) {
		query=query+" and BMR.ActionPerformed='pending' and DATEDIFF(ifnull(ram.ActionedOn,now()),ifnull(ram.CurrentDate,docm.date_of_submission)) <="+ t1+" ";
	} 
	if (tcode.equalsIgnoreCase("2")) {
		query=query+" and BMR.ActionPerformed='pending' and (DATEDIFF(ifnull(ram.ActionedOn,now()),ifnull(ram.CurrentDate,docm.date_of_submission)) >"+ t1+" and DATEDIFF(ifnull(ram.ActionedOn,now()),ifnull(ram.CurrentDate,docm.date_of_submission)) <="+t2+")";
	} 
	if (tcode.equalsIgnoreCase("3")) {
		query=query+" and BMR.ActionPerformed='pending' and DATEDIFF(ifnull(ram.ActionedOn,now()),ifnull(ram.CurrentDate,docm.date_of_submission)) >"+ t2+" ";
	} 
	if (tcode.equalsIgnoreCase("5")) {
		query=query+" and (BMR.ActionPerformed='Finally Approved'  or BMR.ActionPerformed='Forward With Approval' or BMR.ActionPerformed='Returned to RVNL with approval' or BMR.ActionPerformed='Approved')";
	} 
/*	if (ActionPerformed!="") {
...
%>
	<table border=1 class="table table-hover table-bordered"
		...
		<tr class="success">
			...
			<th width="140px">Date Of Submission</th>
			<%
				if (status == "") {
					p2++;
			%>
			...
			if (!tcode.equalsIgnoreCase("5")) {  
			%>
			<th width="90px">Days Pending </th>
			<% } %>
		<th width="90px">Days Since Submission</th>
		...
		String SD=null,Penfrm=null, fdate=null,fdate1=null,SD1=null,Zone_name=null, doctype=null;
	     ...
	     java.util.Date d2 = null;
			...
			while (rs.next()) {
				...
				String pendingsince=null;
				if (action.equalsIgnoreCase("Finally Approved")) {
				pendingsince = "";
				} else {
					pendingsince = rs.getString(18);
				}
				 Zone_name = rs.getString(21);
				String division1 = rs.getString(22);
