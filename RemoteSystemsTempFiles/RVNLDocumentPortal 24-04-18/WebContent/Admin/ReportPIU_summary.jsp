...
<%
	...
	//System.out.println("from stn km"+frmstnkm+"to km"+tostnkm);
		...
		String query1="select zone, division, PIU_of_RVNL,Project,package_name, round(avg(if (document_status='Finally Approved',MOR_Aging,null)),2) AS Sum_mor,  round(avg(if (document_status='Finally Approved',RVNL_Aging,null)),2) AS Sum_rvnl, round(avg(if (document_status='Finally Approved',Org_Aging,null)),2) AS Sum_Org, Count(if (document_status='Finally Approved',1, null)) AS Fin_doc, Count(if (document_status='Pending'  and Type='MOR' and cast(aging as signed integer)<="+t1+",1,null)) AS Pending_doc1, Count(if (document_status='Pending' and Type='MOR' and cast(aging as signed integer) between "+ (t1+1) + " and " +t2+",1,null)) AS Pending_doc2, Count(if (document_status='Pending' and Type='MOR' and cast(aging as signed integer)>"+t2+",1,null)) AS Pending_doc3, Count(if (document_status='Pending' and Type='Others' and cast(aging as signed integer)<="+t1+",1,null)) AS Org_Pending_doc1, Count(if (document_status='Pending' and Type='Others' and cast(aging as signed integer) between "+ (t1+1) + " and " +t2+",1,null)) AS Org_Pending_doc2, Count(if (document_status='Pending' and Type='Others' and cast(aging as signed integer)>"+t2+",1,null)) AS Org_Pending_doc3, Count(if (document_status='Pending' and Type='RVNL' and cast(aging as signed integer)<="+t1+",1,null)) AS rvnl_Pending_doc1, Count(if (document_status='Pending' and Type='RVNL' and cast(aging as signed integer) between "+ (t1+1) + " and " +t2+",1,null)) AS rvnl_Pending_doc2, Count(if (document_status='Pending' and Type='RVNL' and cast(aging as signed integer)>"+t2+",1,null)) AS rvnl_Pending_doc3, sum(if (document_status='Finally Approved',MOR_Aging,null)) AS sSum_mor,  sum(if (document_status='Finally Approved',RVNL_Aging,null)) AS sSum_rvnl,sum(if (document_status='Finally Approved',Org_Aging,null)) AS sSum_Org  from (" + query+ ") as pkqry group by  zone, division, PIU_of_RVNL,Project,package_name ";
	...
	<table border=1 class="table table-hover table-bordered"
		...
		<tr class="success">
			...
			<th width="120px">NUA*-Other Orgn (<= <%=t1 %> days)</th>
			<th width="120px">NUA*-Other Orgn (<%=t1+1 %> to <%=t2 %> days)</th>
			<th width="120px">NUA*-Other Orgn (> <%=t2 %> days)</th>
...
c++;
		...
		<% int i=1;
				...
				String org_pending1 = rs.getString(13);
		...
		<tr class="active">
			...
			String url21="ReportPIU_Summary_details.jsp?zone="+Zone_name+"&division="+division1+"&PIU_name="+piu+"&project="+proj+"&packno="+packn+"&status=Finally Approved&docType="+docType+"&DateStart="+sDate+"&DateEnd="+eDate+"&t1=0&t2=0&majsec="+majsec2+"&smajsec="+smajsec2+"&frmkm="+frmkm+"&tokm="+tokm+"&frmstn="+frmstn+"&tostn="+tostn+"&frmstnkm="+frmstnkm+"&tostnkm="+tostnkm+"&orgName="+org2;
			String url31=url21.replace(" ","+");
			String url41="ReportPIU_Summary_details.jsp?zone="+Zone_name+"&division="+division1+"&PIU_name="+piu+"&project="+proj+"&packno="+packn+"&status=pending&docType="+docType+"&DateStart="+sDate+"&DateEnd="+eDate+"&t1="+t1+"&t2="+t2+"&majsec="+majsec2+"&smajsec="+smajsec2+"&frmkm="+frmkm+"&tokm="+tokm+"&frmstn="+frmstn+"&tostn="+tostn+"&frmstnkm="+frmstnkm+"&tostnkm="+tostnkm+"&orgName="+org2;
			...
			if (sum_rvnl!=null) 
			{%>
			<td><center><%=sum_rvnl%></center></td>
			<% } else  	{%>
			<td><center>-</center></td>
			<% }
			if (sum_orgl!=null) 
			{%>
			<td><center><%=sum_orgl%></center></td>
			<% } else 	{%>
			<td><center>-</center></td>
			<% }
		...
		if (pending3.equalsIgnoreCase("0")) {
			...
			%>
				...
				<% } 
					if (org_pending1.equalsIgnoreCase("0")) {
				%>
					<td><center>-</center></td>
				<%	} else {
				%>
					<td><center><a href="<%=url51 %>&tcode=11"><%=org_pending1 %></a></center></td>
					<% } 
				
			if (org_pending2.equalsIgnoreCase("0")) {
			%>
				<td><center>-</center></td>
			<%	} else {
			%>
				<td><center><a href="<%=url51 %>&tcode=21"><%=org_pending2 %></a></center></td>
				<% } 
			
		if (org_pending3.equalsIgnoreCase("0")) {
			%>
				<td><center>-</center></td>
			<%	} else {
			%>
				<td><center><a href="<%=url51 %>&tcode=31"><%=org_pending3 %></a></center></td>
				<% } 
		...
		if (ssum_mor!=null) 
		{%>
		<td><center><%=ssum_mor%></center></td>
		<% } else {%>
		<td><center>-</center></td>
		<% }
		if (ssum_rvnl!=null) 
		{%>
		<td><center><%=ssum_rvnl%></center></td>
		<% } else  	{%>
		<td><center>-</center></td>
		<% }
		if (ssum_org!=null) 
		{%>
		<td><center><%=ssum_org%></center></td>
		<% } else 	{%>
		<td><center>-</center></td>
		<% }
